  ...
  <div class="qna-page">
    ...
    <div class="qna-wrapper">
      <div class="qna-container">
        ...
        <!-- template 내부 조건부 버튼 렌더링 -->
        <div class="board-controls">
          <RouterLink v-if="currentUser?.role === 'ADMIN'" to="/notice/write" class="write-btn">
            공지 작성
          </RouterLink>
        </div>
        ...
        <table class="board-table">
          ...
          <tbody>
            <tr v-for="post in posts" :key="post.pid">
              <td>{{ post.pid }}</td>
              <td>
                <RouterLink :to="`/notice/detail/${post.pid}`">{{ post.title }}</RouterLink>
              </td>
...
async function fetchPosts() {
  try {
    const response = await aptAi.get('/api/v1/posts/category/notice')
    posts.value = response.data.data
  } catch (error) {
    console.error('공지사항 게시판 불러오기 실패:', error)
  }
}

async function fetchCurrentUser() {
  try {
    const response = await aptAi.get('/api/user/me') // ✅ 토큰 자동 첨부됨
